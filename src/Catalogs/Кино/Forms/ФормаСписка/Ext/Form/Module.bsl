...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ОткрытьФорму("Справочник.Кино.Форма.ФормаЭлемента",Новый структура("Ключ",ВыбраннаяСтрока));
КонецПроцедуры
...
Процедура Удалить(Команда)
	...
	Если НЕ ТС = Неопределено Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ПередЗакрытиемЗавершение", ЭтотОбъект,ТС), Нстр("ru = 'Удалить ""'; en = 'Delete ""'") + Строка(ТС) + """?", РежимДиалогаВопрос.ДаНет,,,Нстр("ru = 'Удаление'; en = 'Removal'"));	 
	КонецЕсли;
КонецПроцедуры
