...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	Дубль = ПроверитьНаДубль(Объект.Ссылка,Объект.КодКинопоиск, Объект.КодIMDB);
	Если НЕ Дубль = Неопределено Тогда
		Отказ = Истина;
		ТекстВопроса = Нстр("ru = 'Ошибка! Кино с таким кодом уже существует. Открыть существующее кино без записи текущего?';
					|en = 'Error! A movie with this code already exists. Open an existing movie without saving the current one?'");
		ПоказатьВопрос(Новый ОписаниеОповещения("ПоказатьВопросДубль", ЭтотОбъект,Дубль), ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	КонецЕсли;	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ПроверитьНаДубль(Ссылка,КодКинопоиск,КодIMDB)
	...
	|ГДЕ
	|	Кино.Ссылка <> &Ссылка
	|	И (Кино.КодIMDB = &КодIMDB
	|			ИЛИ Кино.КодКинопоиск = &КодКинопоиск)");
	Запрос.УстановитьПараметр("Ссылка",Ссылка);
	Запрос.УстановитьПараметр("КодКинопоиск",КодКинопоиск);
	Запрос.УстановитьПараметр("КодIMDB",КодIMDB);
	ЗВ = Запрос.Выполнить();
